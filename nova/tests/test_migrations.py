...
class TestNovaMigrations(BaseMigrationTestCase, CommonTestsMixIn):
    ...
    def _pre_upgrade_185(self, engine):
        fake_instances = [
            dict(uuid='mig185_uuid-1', image_ref='fake_image_1',
                 root_device_name='/dev/vda'),
            dict(uuid='mig185_uuid-2', image_ref='',
                 root_device_name='vda'),
            dict(uuid='mig185_uuid-3', image_ref='fake_image_2',
                 root_device_name='/dev/vda'),
        ]

        fake_bdms = [
            # Instance 1 - image, volume and swap
            dict(instance_uuid='mig185_uuid-1', device_name='/dev/vdc',
                 volume_id='fake_volume_1'),
            dict(instance_uuid='mig185_uuid-1', device_name='/dev/vdb',
                 virtual_name='swap'),
            # Instance 2 - no image. snapshot and volume
            dict(instance_uuid='mig185_uuid-2', device_name='/dev/vda',
                 snapshot_id='fake_snap_1', volume_id='fake_volume_2'),
            dict(instance_uuid='mig185_uuid-2', device_name='/dev/vdc',
                 volume_id='fake_volume_3'),
            # Instance 3 - ephemerals and swap
            dict(instance_uuid='mig185_uuid-3', device_name='/dev/vdc',
                 virtual_name='ephemeral0'),
            dict(instance_uuid='mig185_uuid-3', device_name='/dev/vdd',
                 virtual_name='ephemeral1'),
            dict(instance_uuid='mig185_uuid-3', device_name='/dev/vdb',
                 virtual_name='swap'),
        ]
    ...
    def _check_185(self, engine, data):
        block_device = db_utils.get_table(engine, 'block_device_mapping')

        instance_qs = []

        for instance in ('mig185_uuid-1', 'mig185_uuid-2', 'mig185_uuid-3'):
