...
class BootFromVolumeTest(test.TestCase):
    ...
    def setUp(self):
        ...
        self._block_device_mapping_v2_seen = None
        self.flags(
    ...
    def _get_fake_compute_api_create(self):
        def _fake_compute_api_create(cls, context, instance_type,
            ...
            self._block_device_mapping_v2_seen = kwargs.get(
                'block_device_mapping')
    ...
    def test_create_root_volume_bdm_v2(self):
        ...
        self.assertEqual(FAKE_UUID, server['id'])
        self.assertEqual(CONF.password_length, len(server['adminPass']))
        self.assertEqual(len(self._block_device_mapping_v2_seen), 1)
        self.assertEqual(self._block_device_mapping_v2_seen[0]['volume_id'], 1)
        self.assertEqual(self._block_device_mapping_v2_seen[0]['boot_index'],
                         0)
        self.assertEqual(self._block_device_mapping_v2_seen[0]['device_name'],
                '/dev/vda')
