...
SUPPORTED_DEVICE_TYPES = ('disk', 'cdrom', 'floppy', 'mmc')
...
def get_info_from_bdm(virt_type, bdm, mapping, disk_bus=None,
    ...
    device_name = get_device_name(bdm)

    bdm_bus = bdm.get('disk_bus') or disk_bus
    if not is_disk_bus_valid_for_virt(bdm_bus, virt_type):
    ...
    if not device_name:
        if assigned_devices:
            pre_assinged_mapping = dict((dev, {'dev': dev})
                for dev in assigned_devices)
            mapping = pre_assinged_mapping.update(mapping)

        device_name = find_disk_dev_for_disk_bus(mapping, bdm_bus)
...
def get_root_info(virt_type, image_meta, root_bdm, disk_bus, cdrom_bus,
    ...
    if not root_bdm or bdm.get('image_id'):
        ...
        root_device_name = get_device_name(root_bdm)
...
def default_device_names(instance, root_device_name, update_func,
    ...
    get_disk_mapping(virt_type, instance, disk_bus, cdrom_bus,
    ...
    for driver_bdm in itertools.chain(block_device_info['ephemerals'],
        ...
        if driver_bdm.id in devices:
            bdm = missing_devices[driver_bdm.id]
            ...
            bdm['device_type'] = driver_bdm['device_type']
            update_func(bdm)
...
def update_bdm(bdm, info):
    ...
    bdm.update(
        dict(zip((device_name_field, 'disk_bus', 'device_type'),
                 (info['dev'], info['bus'], info['type'])))
    )
...
def get_disk_mapping(virt_type, instance,
    ...
    mapping['root'] = get_root_info(virt_type, image_meta,
                                    root_bdm, mapping)
    if not root_bdm:
        mapping['disk'] = root_info
    ...
    for idx, eph in enumerate(driver.block_device_info_get_ephemerals(
        ...
        eph_info = get_info_from_bdm(
            virt_type, bdm, mapping, disk_bus,
