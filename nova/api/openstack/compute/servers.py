    ...
    def create(self, req, body):
        ...
        if self.ext_mgr.is_loaded('os-volumes'):
            ...
            for bdm in block_device_mapping:
                try:
                    block_device.validate_device_name(bdm["device_name"])
                except exception.InvalidBDMFormat:
                    expl = _("Invalid device name found in the "
            ...
            #                   block_devuices in the same request
            if block_device_mapping and block_device_mapping_v2:
                expl = _('Using different block_device_mapping syntaxes '
                         'is not allowed in the same request.')
                raise exc.HTTPBadRequest(explanation=expl)

            # Decide which format was passed and
            # set apropriate flags to pass to create
            legacy_bdm = bool(block_device_mapping)
            ...
            try:
                ...
                expl = _('Invalid block device mapping format.')
                raise exc.HTTPBadRequest(explanation=expl)
